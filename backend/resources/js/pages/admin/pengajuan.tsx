...
const STATUS_LABEL: Record<string, string> = {
    ...
    disetujui: 'Disetujui', ditolak_kepala: 'Ditolak Kepala', selesai: 'Selesai', dibatalkan: 'Dibatalkan',
};
...
export default function AdminPengajuan({ pengajuan, filters }: Props) {
    const [search, setSearch] = useState(filters.search ?? '');

    const applyFilter = (extra: Record<string, string>) =>
        router.get('/admin/pengajuan', { ...filters, ...extra }, { preserveState: true });

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Pengajuan Surat | SADESA" />

            <div className="flex flex-col gap-6 p-4">
                <div>
                    <h1 className="text-xl font-bold text-foreground">Pengajuan Surat</h1>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    {(filters.status || filters.search) && (
                        <button onClick={() => router.get('/admin/pengajuan')} className="rounded-lg border px-3 py-2 text-sm text-muted-foreground hover:bg-muted">Reset</button>
                    )}
                </div>
                ...
                <div className="rounded-xl border bg-card shadow-sm">
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {pengajuan.data.length === 0 ? (
                                    <tr><td colSpan={6} className="py-12 text-center text-muted-foreground">Tidak ada data.</td></tr>
                                    ...
                                    <tr key={item.id} className="border-b last:border-0 hover:bg-muted/20">
                                        ...
                                        <td className="px-4 py-3">
                                            <Link href={`/admin/pengajuan/${item.id}`} className="inline-flex items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs hover:bg-muted">
                    ...
                    {pengajuan.last_page > 1 && (
                        <div className="flex items-center justify-between border-t px-4 py-3">
                            <p className="text-sm text-muted-foreground">Halaman {pengajuan.current_page} dari {pengajuan.last_page}</p>
